    ...
    try {
 	...
 	stage('NPM Install') {
    	...
    	stage('Please choose Testing or Not') {
            ...
            if( "${USER_INPUT}" == "Smoke"){
                 stage('Smoke'){
                  	   sh 'curl -u amitsingharoy:11b9e2fdaafae4ee5a9745c2b4eb3b8063 http://192.168.1.158:8080/job/CloudBLM_Smoke_Test/build?token=smoke'
                  }
    	...
    	stage('Please choose Regression or Not') {
            ...
            if( "${USER_INPUT}" == "Regression"){
                 stage('Regression'){
               sh "curl -u amitsingharoy:11b9e2fdaafae4ee5a9745c2b4eb3b8063 http://192.168.1.158:8080/job/CloudBLM_Regression_Test/build?token=regression"
                  }
